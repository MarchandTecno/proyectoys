...
@api_rutas.route('/Obtener-todas-tareas', methods=['GET'])
def obtener_todas_tareas_ruta():
    id_proyecto = request.args.get('idProyecto')

    # Llamar a la función para obtener todas las tareas
    resultado = obtener_todas_tareas(id_proyecto)

    # Verificar si se encontraron tareas y devolver la respuesta adecuada
    if 'error' in resultado:
        return jsonify({'error': resultado['error']}), 404
    else:
        return jsonify(resultado), 200
